...
\abstract{
When we process a speaker's utterance, we use more than the information in the speech signal. Because linguistic communication is inherently noisy---sometimes we misspeak, sometimes we mishear---adult listeners integrate bottom-up cues from speech perception with top-down expectations about what speakers are likely to say. Further, in line with the predictions of ideal observer models, adults flexibly adapt how much they rely on these two kinds of cues in proportion to their changing reliability. Do children also process language in the same flexible, expectation-based way? We presented 4- and 5-year-old children with ambiguous utterances afforded two different interpretations depending on whether they privileged perceptual input or linguistic expectations. We then manipulated both the reliability of the perceptual input and of their speaker-expectations. As predicted by a noisy channel model of speech processing, children flexibly adapted their reliance on each of these cues as it changed in reliability.}
...
This same phenomenon begins to offer an explanation for why children appear to integrate information in an adult-like way in some language processing tasks but not others. A priori, we would predict that children's should use their model-based expectations when they have reliable models in a given domain. Thus, children should and do integrate when the relevant models are of speech sounds, speakers, or semantic plausibility \cite<e.g.,>{clark2009, creel2012, harris2012}. On the other hand, children fail in cases where the relevant models are of quantifier semantics and complex syntactic constructions \cite<e.g.,>{trueswell1999, noveck2001}. However, based on prior evidence that children's understanding of quantifiers is limited, as is their exposure to relative clauses, this is exactly the result a noisy channel framework for speech processing would predict  \cite{barner2011, montag2015}.

The noisy channel framework thus offers a potential unifying framework for understanding both children's and adults' language processing systems, as well as how one develops from the other. The processing that unfolds is well characterized by the interaction between two models: one of perception, and one of expectation. But what is the content of these models? One possibility is that expectations are fundamentally about speakers, and that linguistic communication is essentially pragmatic all the way down. Alternatively, these models could be linguistic in content, about the distributions of words, referents, and their mutual predictability. We hope that the paradigm we have developed here will be a powerful tool for helping to resolve this fundamental tension. 
