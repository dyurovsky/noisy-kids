...
In order to validate our manipulation, we first analyze the effect of the speaker on Exposure trials. If participants were attending to the Speaker's requests during Exposure trials, those in the Plausible condition should have chosen the plausible referent (kittens) and those in the Implausible condition should have chosen the implausible referent. We provide two convergent analysis that suggest that they did. 
...
When we use language to communicate, we are doing more than processing the sounds hear; we are trying to infer the speaker's intended meaning \cite{clark1996}. Because of the noise inherent in linguistic communication, a lot of work for this inference is done by our expectations about what speakers are likely to want to say \cite{grice1975, frank2012}. But how much should listeners rely on the acoustic signal and how much should they rely on their expectations? Ideal observer models of this kind of cue integration predict that each cue should be weighted in proportion to it's reliability \cite{jacobs1999, ernst2002}. This prediction appears to be borne out in adult listeners across domains in language acquisition from phonology to syntax \cite<e.g.,>{gibson2013, mcclelland2006, kleinschmidt2015}.

The experiments in this paper provide the first evidence that this adaptive expectation-based language processing is already available to 4-5 year old children. Not only do preschoolers combine their expectations about a speaker's likely utterance with the speech they are hearing, they flexibly re-weigh their reliance on each as they develop stronger expectations about the speaker and as the speech signal changes in clarity.

Interestingly, while the effect size of this adaptation was nearly identical in both children and adults, their baseline reliance on perceptual vs. linguistic cues was not. Overall, children hearing the same ambiguous utterances were more likely to rely more on their linguistic expectations. Because of the greater noise inherent in young children's perceptual processing systems, the same acoustic stimulus may effectively less reliable for children than adults
\cite{neuman1983, lyons2011}. On this basis, the noisy channel framework makes a clear prediction about integration in atypical populations. Children whose acoustic processing is impaired, (e.g. children with cochlear implants) should rely relatively more on their expectations, whereas children with impaired language models (e.g. children with specific language impairment) should rely relatively more the acoustic signal.

This same phenomenon begins to offer an explanation for why children appear to integrate information in an adult-like way in some language processing tasks but not others. A priori, we would predict that children's use of their model-based expectations when they have reliable models in a given domain. Thus, children should and do integrate when the relevant models are of speech sounds, speakers, or semantic plausibility \cite<e.g.,>{clark2009, creel2012, harris2012}. On the other hand, children fail in cases where the relevant models are of quantifier semantics and complex syntactic constructions \cite<e.g.,>{trueswell1999, noveck2001}. However, based on prior evidence that children's understanding of quantifiers is limited, as is their exposure to relative clauses, this is exactly the result a noisy channel framework for speech processing would predict  \cite{barner2011, montag2015}.

The noisy channel framework thus offers a potential unifying framework for understanding both children's and adults' language processing systems, as well as how one develops from the other. The processing that unfolds is well characterized by the interaction between two models: one of perception, and one of expectation. But what is the content of these models? One possibility is that expectations are fundamentally about speakers, and that linguistic communication is essentially pragmatic all the way down. Alternatively, these models could be linguistic in content, about the distribution of words, reference, and their mutual predictability. We hope that the paradigm we have developed here will be a powerful tool for helping to resolve this fundamental tension. 
