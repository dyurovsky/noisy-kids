...
read.child <- function(filename) {
  ...
  split.frame$trial <- sort(rep(1:12,16))
...
child.long.data <- bind_rows(lapply(child.files,read.child)) %>%
  ...
  mutate(condition = factor(ifelse(order == "0", "Normal", "Implausible")),
         trial.type = factor(ifelse(trial <= 8, "Familiar", "Novel")),
         Literal = ifelse(value == "Y", 1, 0))
...
ns <- all.child.long.data %>% 
  ...
  distinct() %>% group_by(experiment,condition, trial.type) %>% 
  summarise(n = n())
...
Plot
```{r child_plot,fig.height=5,fig.width=8}
