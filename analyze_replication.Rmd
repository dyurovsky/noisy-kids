...
child.overwrite.data %<>%
  ...
  select(experiment,subj,age,gender,condition,trial.type,word,Literal)
```
...
# Age differnces?
age.data <- select(new.child.data, subj, condition, age, gender) %>%
  distinct(subj) %>%
  group_by(condition)

condition_demos <- age.data %>%
  group_by(condition) %>%
  summarise(n = n(),
...
plotting.age.data <- multi_boot_standard(age.data, 'age')

ggplot(plotting.age.data, aes(x=condition, y = mean)) +
  geom_histogram(stat = "identity", fill = "steelblue") +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper))


lm.child.v1 <- glmer(Literal ~ speaker.type + noise.type + test.type + 
                       (1|word) + (1|subj),
                  family = "binomial", data = filter(new.child.data,trial.type == "Test"))

summary(lm.child.v1)

lm.child.v2 <- glmer(Literal ~ condition  +  (1|word) + (1|subj),
                  family = "binomial", data = filter(new.child.data,trial.type == "Test"))
...
lm.child.v2 <- glmer(Literal ~ condition  +  (1|word) + (1|subj),
                  family = "binomial", 
                  data = filter(new.child.data, experiment == "replication",
                                trial.type == "Test", 
                                condition %in% c("Implausible", "No Noise Implausible")))

summary(lm.child.v2)


lm.child.v2 <- glm(Literal ~ condition,
                  family = "binomial", 
                  data = filter(new.child.data, experiment == "replication",
                                trial.type == "Test", 
                                condition %in% c("Implausible", "No Noise Implausible")))
...
implausible.data <- new.child.data %>%
  filter(experiment == "replication",
         trial.type == "Test", 
         condition %in% c("Implausible", "No Noise Implausible")) %>%
  group_by(condition,subj) %>%
  summarise(Literal = sum(Literal))

ggplot(aes(x = Literal, fill = condition), data = implausible.data) +
...
demos <- all.long.data %>% 
  ungroup() %>% 
  select(experiment,trial.type,condition,subj,age) %>% 
  distinct() %>% 
  group_by(experiment,condition, trial.type) %>% 
  summarise(n = n(),
            mean.age = mean(age,na.rm = TRUE))

kable(filter(demos, trial.type == "Test"))
...
Plot Separately
```{r plot_separate,fig.height=5,fig.width=9, cache = FALSE}
